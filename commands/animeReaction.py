...
class AnimeReaction:
    def __init__(self, client: CustomClient):
        ...
        client.addCommandDescription("reaction", "Send anime reaction gif", ";reaction {reaction}")
        ...
        def command(_, message: pyrogram.types.Message):
            reaction_list = ['waifu','neko','shinobu','megumin','bully','cuddle','cry','hug','awoo','kiss','lick','pat','smug','bonk','yeet','blush','smile','wave','highfive','handhold','nom','bite','glomp','slap','kill','kick','happy','wink','poke','dance','cringe']

            print(message)
            try:
                arg = message.command[1]
            except:
                return message.reply(f"Non argument; list: {reaction_list}")
